// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"bytes"
	"context"
	"encoding/json"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"
)

// AzureDimensionDiscoveryMethod azure dimension discovery method
//
// swagger:model AzureDimensionDiscoveryMethod
type AzureDimensionDiscoveryMethod struct {

	// dimension name
	// Required: true
	DimensionName *string `json:"dimensionName"`

	// metric name
	MetricName string `json:"metricName,omitempty"`

	// period
	// Required: true
	Period *string `json:"period"`
}

// Name gets the name of this subtype
func (m *AzureDimensionDiscoveryMethod) Name() string {
	return "AzureDimensionDiscoveryMethod"
}

// SetName sets the name of this subtype
func (m *AzureDimensionDiscoveryMethod) SetName(val string) {
}

// UnmarshalJSON unmarshals this object with a polymorphic type from a JSON structure
func (m *AzureDimensionDiscoveryMethod) UnmarshalJSON(raw []byte) error {
	var data struct {

		// dimension name
		// Required: true
		DimensionName *string `json:"dimensionName"`

		// metric name
		MetricName string `json:"metricName,omitempty"`

		// period
		// Required: true
		Period *string `json:"period"`
	}
	buf := bytes.NewBuffer(raw)
	dec := json.NewDecoder(buf)
	dec.UseNumber()

	if err := dec.Decode(&data); err != nil {
		return err
	}

	var base struct {
		/* Just the base type fields. Used for unmashalling polymorphic types.*/

		Name string `json:"name"`
	}
	buf = bytes.NewBuffer(raw)
	dec = json.NewDecoder(buf)
	dec.UseNumber()

	if err := dec.Decode(&base); err != nil {
		return err
	}

	var result AzureDimensionDiscoveryMethod

	if base.Name != result.Name() {
		/* Not the type we're looking for. */
		return errors.New(422, "invalid name value: %q", base.Name)
	}

	result.DimensionName = data.DimensionName
	result.MetricName = data.MetricName
	result.Period = data.Period

	*m = result

	return nil
}

// MarshalJSON marshals this object with a polymorphic type to a JSON structure
func (m AzureDimensionDiscoveryMethod) MarshalJSON() ([]byte, error) {
	var b1, b2, b3 []byte
	var err error
	b1, err = json.Marshal(struct {

		// dimension name
		// Required: true
		DimensionName *string `json:"dimensionName"`

		// metric name
		MetricName string `json:"metricName,omitempty"`

		// period
		// Required: true
		Period *string `json:"period"`
	}{

		DimensionName: m.DimensionName,

		MetricName: m.MetricName,

		Period: m.Period,
	})
	if err != nil {
		return nil, err
	}
	b2, err = json.Marshal(struct {
		Name string `json:"name"`
	}{

		Name: m.Name(),
	})
	if err != nil {
		return nil, err
	}

	return swag.ConcatJSON(b1, b2, b3), nil
}

// Validate validates this azure dimension discovery method
func (m *AzureDimensionDiscoveryMethod) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateDimensionName(formats); err != nil {
		res = append(res, err)
	}

	if err := m.validatePeriod(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *AzureDimensionDiscoveryMethod) validateDimensionName(formats strfmt.Registry) error {

	if err := validate.Required("dimensionName", "body", m.DimensionName); err != nil {
		return err
	}

	return nil
}

func (m *AzureDimensionDiscoveryMethod) validatePeriod(formats strfmt.Registry) error {

	if err := validate.Required("period", "body", m.Period); err != nil {
		return err
	}

	return nil
}

// ContextValidate validate this azure dimension discovery method based on the context it is used
func (m *AzureDimensionDiscoveryMethod) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

// MarshalBinary interface implementation
func (m *AzureDimensionDiscoveryMethod) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *AzureDimensionDiscoveryMethod) UnmarshalBinary(b []byte) error {
	var res AzureDimensionDiscoveryMethod
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
